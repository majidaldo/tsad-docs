...
\end{figure}
\noindent
Bias terms are omitted for clarity but are included as elements in $\vc{\theta}$ along with elements from $\W$. The notation of $\vc{\theta}$ follows that of $\mt{W}$ but captures more variables by vectorizing each component for concatenation into a single vector. $\vc{\theta}_{ss} = (\W_{ss},\vc{b}_s)$, $\vc{\theta}_{xs} = (\W_{xs})$, and $\vc{\theta}_{so} = (\W_{so},\vc{b}_o)$ (mathematical operations to produce the vector are implied).
...
An involved application of the (backpropagated) chain rule to the loss function using the \emph{basic} (unfolded) RNN defined in Equations \ref{eqn:rnn} leads to Equation \ref{eqn:dLdtheta}.
...
\end{equation}
\noindent
Equation \ref{eqn:dldtheta} shows that there are multiplicative `chains' of differentials for each previous time step.


%todo vector notation for W in  theta= ( ->W, .. ,...)
