...
%
For example, Hessian-free optimization \cite{Martens2010a} uses second order information that can make use of the ratio of small gradients to small curvature to more directly find optimal parameters.
%
...
English-to-French translation  \cite{Sutskever2014},
...
%
The LSTM variant presented in Figure \ref{fig:lstmfig} is found in \cite{Graves2013c}.
...
%
Furthermore, $\mt{W}_{ci}$, $\mt{W}_{cf}$, and $\mt{W}_{co}$ are diagonal matrices so the number of weights associated with each multiplication by $\vc{c}\tm{t-1}$ equals $v$.

The size that can be attributed to the LSTM module is the dimension of $\vc{i}$, $\vc{f}$, $\vc{c}$, $\vc{o}$, or $\vc{h}$ which are equal.


Very briefly, the central element in a LSTM module is the `memory cell', $\vc{c}$, that works to keep information unchanged over many time steps (can be called a conveyor belt).
%
