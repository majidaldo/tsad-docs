...
\tikzset{
  ...
  ,zone/.style={rectangle,dashed,draw=black,inner sep=.2in}
}
...
\node[right of=vagrant] (vagrant2) {
  \begin{tikzpicture}[tikzcomputerpic]
    ...
    \begin{pgfonlayer}{background}
      \node[computer,fit=(vag2nm) (stack)] (vag2box) {};
    \end{pgfonlayer}
  \end{tikzpicture}
};


\node[right of=vagrant2] (aws1) {
  \begin{tikzpicture}[tikzcomputerpic]
    % vagrant compute computer
    \matrix[stack](stack){
...
\node[right of=aws1] (aws2) {
  \begin{tikzpicture}[tikzcomputerpic]
    % init computer
    \matrix[stack](stack){
      \node[dockerapp] (asw2app4) {app4};
      \\
...
%location
\node[zone
,fit=(init) (vagrant) (local) (vagrant2)
]
{local};
...
\connectorH[thick]{.5}{initshare}{localshare}
