...
%
More specifically, the (overall) training procedure is outlined in the following box.
...
\STATE{} \COMMENT{update parameters according to RMSprop\cite{Graves2013b}
(per parameter, indices to $p$ omitted)}
\STATE{
  \begin{flalign*}
    f_{i+1}
    &\gets 
...
\STATE \COMMENT{stop when no improvement patience times}
\IF{
...
The \texttt{theanets} \cite{Johnson2015} implementation of RNNs
\footnote{With every calculation of $\vc{o}$, the RNN states are initialized to 0.}.
%
\texttt{theanets} is based on the computational graph compiler \texttt{theano} \cite{Bergstra2010}.
%
