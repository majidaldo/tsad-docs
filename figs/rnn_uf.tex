\documentclass[]{standalone}
\input{../custom}

\usepackage{pgf}
\usepackage{tikz}
\usetikzlibrary{arrows,automata,matrix}
\usepackage[latin1]{inputenc}
\begin{document}
\begin{tikzpicture}[->,>=stealth'
	,shorten >=1pt
	,auto
	,node distance=2.8cm,
              semithick]
  \tikzstyle{every state}
  =[minimum size={10pt+width("$x^{(t-1)}$")}]

  \matrix (m) [matrix of nodes
  ,row sep=.25in,column sep=.5in] {
    % 3 ys
    &
    \node[](ytm1) {$\vc{y}^{(t-1)}$}; &
    \node[](yt) {$\vc{y}^{(t)}$};    &
    \node[](ytp1) {$\vc{y}^{(t+1)}$}; 
    \\
    % 3 Ls
    &
    \node[state](Ltm1) {$L^{(t-1)}$}; &
    \node[state](Lt) {$L^{(t)}$}; &
    \node[state](Ltp1) {$L^{(t+1)}$};
    \\
    % 3 os
    &
    \node[state](otm1) {$\vc{o}^{(t-1)}$}; &
    \node[state](ot) {$\vc{o}^{(t)}$}; &
    \node[state](otp1) {$\vc{o}^{(t+1)}$};
    \\
    % 5 hns
    \node[state,dashdotted](hndm) {$\vc{h}_n^{(\ldots)}$}; &
    \node[state,dotted](hntm1) {$\vc{h}_n^{(t-1)}$}; &
    \node[state,dotted](hnt) {$\vc{h}_n^{(t)}$}; &
    \node[state,dotted](hntp1) {$\vc{h}_n^{(t+1)}$}; &
    \node[state,dashdotted](hndp) {$\vc{h}_n^{(\ldots)}$};
    \\
    % 5 h2s
    \node[state,dashed](h2dm) {$\vc{h}_2^{(\ldots)}$}; &
    \node[state](h2tm1) {$\vc{h}_2^{(t-1)}$}; &
    \node[state](h2t) {$\vc{h}_2^{(t)}$}; &
    \node[state](h2tp1) {$\vc{h}_2^{(t+1)}$}; &
    \node[state,dashed](h2dp) {$\vc{h}_2^{(\ldots)}$};
    \\
    % 5 h1s
    \node[state,dashed](h1dm) {$\vc{h}_1^{(\ldots)}$}; &
    \node[state](h1tm1) {$\vc{h}_1^{(t-1)}$}; &
    \node[state](h1t) {$\vc{h}_1^{(t)}$}; &
    \node[state](h1tp1) {$\vc{h}_1^{(t+1)}$}; &
    \node[state,dashed](h1dp) {$\vc{h}_1^{(\ldots)}$}; 
    \\
    % 3 xs
    &
    \node[](xtm1) {$\vc{x}^{(t-1)}$}; &
    \node[](xt) {$\vc{x}^{(t)}$};    &
    \node[](xtp1) {$\vc{x}^{(t+1)}$};
    \\
  };
  % left to right, top to bottom.
  % each layer outgoing and within itself arrows
  % minus ... 
  \path [dashdotted] (hndm)  edge  node {$\mt{W}_{h_nh_n}$} (hntm1);
  \path [dashed] (h2dm)  edge  node {$\mt{W}_{h_2h_2}$} (h2tm1);
  \path [dashed] (h1dm)  edge  node {$\mt{W}_{h_1h_1}$} (h1tm1);
  % t-1
  \path [      ] (ytm1)  edge  node {     }      (Ltm1);
  \path [      ] (otm1)  edge  node {     }      (Ltm1);
  \path [dotted] (hntm1) edge  node {$\mt{W}_{h_nh_o}$}   (otm1);
  \path [      ] (h2tm1) edge  node {}           (hntm1);
  \path [      ] (h1tm1) edge  node {}           (h2tm1);
  \path [      ] (xtm1)  edge  node {$\mt{W}_{xh_1}$}   (h1tm1);
  \path [dotted] (hntm1) edge  node {$\mt{W}_{h_nh_n}$} (hnt);
  \path [      ] (h2tm1) edge  node {$\mt{W}_{h_2h_2}$} (h2t);
  \path [      ] (h1tm1) edge  node {$\mt{W}_{h_1h_1}$} (h1t);
  % t
  \path [      ] (yt)    edge  node {     }      (Lt);
  \path [      ] (ot)    edge  node {     }      (Lt);
  \path [dotted] (hnt)   edge  node {$\mt{W}_{h_nh_o}$}   (ot);
  \path [      ] (h2t)   edge  node {}           (hnt);
  \path [      ] (h1t)   edge  node {}           (h2t);
  \path [      ] (xt)    edge  node {$\mt{W}_{xh_1}$}   (h1t);
  \path [dotted] (hnt)   edge  node {$\mt{W}_{h_nh_n}$} (hntp1);
  \path [      ] (h2t)   edge  node {$\mt{W}_{h_2h_2}$} (h2tp1);
  \path [      ] (h1t)   edge  node {$\mt{W}_{h_1h_1}$} (h1tp1);
  % t+1
  \path [      ] (ytp1)  edge  node {     }      (Ltp1);
  \path [      ] (otp1)  edge  node {     }      (Ltp1);
  \path [dotted] (hntp1) edge  node {$\mt{W}_{h_nh_o}$}   (otp1);
  \path [      ] (h2tp1) edge  node {}           (hntp1);
  \path [      ] (h1tp1) edge  node {}           (h2tp1);
  \path [      ] (xtp1)  edge  node {$\mt{W}_{xh_1}$}   (h1tp1);
  % plus ...
  \path [dotted] (hntp1) edge  node {$\mt{W}_{h_nh_n}$} (hndp);
  \path [      ] (h2tp1) edge  node {$\mt{W}_{h_2h_2}$} (h2dp);
  \path [      ] (h1tp1) edge  node {$\mt{W}_{h_1h_1}$} (h1dp);

  
\end{tikzpicture}

\end{document}