...
\begin{tikzpicture}[->,>=stealth'
  ...
  % minus ... 
  \path [dashdotted] (hndm)  edge  node {$\mt{\mathcal{W}}_{h_nh_n}$} (hntm1);
  \path [dashed] (h2dm)  edge  node {$\mt{\mathcal{W}}_{h_2h_2}$}     (h2tm1);
  \path [dashed] (h1dm)  edge  node {$\mt{\mathcal{W}}_{h_1h_1}$}     (h1tm1);
  % t-1
  \path [      ] (ytm1)  edge  node {     }                 (Ltm1);
  \path [      ] (otm1)  edge  node {     }                 (Ltm1);
  \path [dotted] (hntm1) edge  node {$\mt{W}_{h_nh_o}$}     (otm1);
  \path [      ] (h2tm1) edge  node {}                      (hntm1);
  \path [      ] (h1tm1) edge  node {}                      (h2tm1);
  \path [      ] (xtm1)  edge  node {$\mt{W}_{xh_1}$}       (h1tm1);
  \path [dotted] (hntm1) edge  node {$\mt{\mathcal{W}}_{h_nh_n}$}     (hnt);
  \path [      ] (h2tm1) edge  node {$\mt{\mathcal{W}}_{h_2h_2}$}     (h2t);
  \path [      ] (h1tm1) edge  node {$\mt{\mathcal{W}}_{h_1h_1}$}     (h1t);
  % t
  \path [      ] (yt)    edge  node {     }                 (Lt);
  \path [      ] (ot)    edge  node {     }                 (Lt);
  \path [dotted] (hnt)   edge  node {$\mt{W}_{h_nh_o}$}     (ot);
  \path [      ] (h2t)   edge  node {}                      (hnt);
  \path [      ] (h1t)   edge  node {}                      (h2t);
  \path [      ] (xt)    edge  node {$\mt{W}_{xh_1}$}       (h1t);
  \path [dotted] (hnt)   edge  node {$\mt{\mathcal{W}}_{h_nh_n}$}     (hntp1);
  \path [      ] (h2t)   edge  node {$\mt{\mathcal{W}}_{h_2h_2}$}     (h2tp1);
  \path [      ] (h1t)   edge  node {$\mt{\mathcal{W}}_{h_1h_1}$}     (h1tp1);
  % t+1
  \path [      ] (ytp1)  edge  node {     }                 (Ltp1);
  \path [      ] (otp1)  edge  node {     }                 (Ltp1);
  \path [dotted] (hntp1) edge  node {$\mt{W}_{h_nh_o}$}     (otp1);
  \path [      ] (h2tp1) edge  node {}                      (hntp1);
  \path [      ] (h1tp1) edge  node {}                      (h2tp1);
  \path [      ] (xtp1)  edge  node {$\mt{W}_{xh_1}$}       (h1tp1);
  % plus ...
  \path [dotted] (hntp1) edge  node {$\mt{\mathcal{W}}_{h_nh_n}$}     (hndp);
  \path [      ] (h2tp1) edge  node {$\mt{\mathcal{W}}_{h_2h_2}$}     (h2dp);
  \path [      ] (h1tp1) edge  node {$\mt{\mathcal{W}}_{h_1h_1}$}     (h1dp);
